...
class CELEBA(data.Dataset):
    def __init__(self, root, split="train", is_transform=False, img_size=(32, 32), augmentations=None):
        ...
        self.files[self.split] = list(map(lambda x: '/'.join([base_path, 'img_align_celeba_png', x]), set(selected_file_names)))
        # print(self.files)
        self.labels[self.split] = list(map(lambda x: label_map[x], set(selected_file_names)))
        ...
        # print(set(map(lambda x:x.split('/')[-1], self.all_files)).intersection(set(selected_file_names)))
